...
class MaritimeMetrics(Metric):
	def __init__(self, cfg):
		self.static_obstacle_class = cfg.SEGMENTATION.STATIC_OBSTACLE_CLASS
		self.dynamic_obstacle_class = cfg.SEGMENTATION.DYNAMIC_OBSTACLE_CLASS
	...
	def compute(self, mask_pred, gt_sem, gt_pan, ann_pan, image_name):
		...
		obstacle_mask = ((gt_pan == self.cfg.SEGMENTATION.STATIC_OBSTACLE_CLASS) | (gt_pan == self.cfg.SEGMENTATION.DYNAMIC_OBSTACLE_CLASS)).astype(np.uint8) # Static obstacles mask

		segment_data = []

		# 1. Dynamic obstacles recall
		valid_preds = (mask_pred == self.dynamic_obstacle_class).astype(np.uint8)
		...
		gt_o = ((gt_sem == self.dynamic_obstacle_class) | gt_sem == self.static_obstacle_class).astype(np.uint8)
		pred_o = ((mask_pred == self.dynamic_obstacle_class)|mask_pred == self.static_obstacle_class).astype(np.uint8)
		...
		fp_mask = ((mask_pred == self.dynamic_obstacle_class)|(mask_pred == self.static_obstacle_class)) * water_mask_e
